...
export const fetchVisualization = async (
  ...
  if (query.period === 'custom') {
    apiQueryPayload.customDateRange = {
      start_date: query.customStartDate,
      end_date: query.customEndDate,
    };
  } else {
    apiQueryPayload.dateRange = query.period;
  }
...
export const fetchAiQuery = async (prompt: string): Promise<ApiResponse> => {
  ...
  try {
    const response = await apiClient.post<ApiResponse>('/query-from-text', {
      prompt,
    });
    return response.data;
  } catch (error) {
